...
void *sum(void *arg) {
    ...
    // Acquire the lock, update the accumulator and release lock
    acquire(args->lock);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        generate_values(N);
        clock_gettime(CLOCK_MONOTONIC, &start_timer); //Start execution time
        create_threads(N, K);
        clock_gettime(CLOCK_MONOTONIC, &finish_timer); //Finish execution time
        execution_time = (finish_timer.tv_sec - start_timer.tv_sec);
        execution_time += (finish_timer.tv_nsec - start_timer.tv_nsec) / 1000000000.0;
